# wxDifferencePlot
# This function is based on .difplotLM.R but with some tiny modifications

wxDifferencePlot <- function(refshape,targetshape,color=4,lwd=1,lcol=2,rgl.new=TRUE, text=TRUE)
{
  if (rgl.new == TRUE)
    open3d()
  A <- refshape
  ...
  sz <- (cSize(refshape)/sqrt(k))*(1/200)
